...
u8 DoFieldEndTurnEffects(void)
{
    ...
    do
    {
        ...
        case ENDTURN_ROCKY_TERRAIN:
            effect = TryEndTerrain(STATUS_FIELD_ROCKY_TERRAIN, B_MSG_TERRAINENDS_ROCKY);
            gBattleStruct->turnCountersTracker++;
            break;
        case ENDTURN_DARK_TERRAIN:
            effect = TryEndTerrain(STATUS_FIELD_DARK_TERRAIN, B_MSG_TERRAINENDS_DARK);
            gBattleStruct->turnCountersTracker++;
            break;
...
static bool32 TryChangeBattleTerrain(u32 battler, u32 statusFlag, u8 *timer)
{
    ...
    {
        gFieldStatuses &= ~(STATUS_FIELD_MISTY_TERRAIN | STATUS_FIELD_GRASSY_TERRAIN | STATUS_FIELD_ELECTRIC_TERRAIN | STATUS_FIELD_PSYCHIC_TERRAIN | STATUS_FIELD_DARK_TERRAIN | STATUS_FIELD_ROCKY_TERRAIN);
...
static u32 CalcAttackStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    {
        if (IS_MOVE_PHYSICAL(move) || (moveType == TYPE_FIRE && GetBattlerAbility(battlerAtk) == ABILITY_FIRE_CORE))
        {
...
static u32 CalcDefenseStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    if (gBattleMoves[move].effect == EFFECT_PSYSHOCK || (IS_MOVE_PHYSICAL(move) && gBattleMoves[move].effect != EFFECT_PRESSUREPOINT)) // uses defense stat instead of sp.def
    {
        defStat = def;
    ...
    // custom: hail def boost for ice
    if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_ICE) && WEATHER_HAS_EFFECT && gBattleWeather & B_WEATHER_HAIL && usesDefStat)
        MulModifier(&modifier, UQ_4_12(1.5));
...
static u16 CalcTypeEffectivenessMultiplierInternal(u16 move, u8 moveType, u8 battlerAtk, u8 battlerDef, bool32 recordAbilities, u16 modifier)
{
    ...
    else if ((moveType == TYPE_GROUND || move == MOVE_VOLCANIC_FLOW) && !IsBattlerGrounded2(battlerDef, TRUE) && !(gBattleMoves[move].flags & FLAG_DMG_UNGROUNDED_IGNORE_TYPE_IF_FLYING))
    {
        modifier = UQ_4_12(0.0);
